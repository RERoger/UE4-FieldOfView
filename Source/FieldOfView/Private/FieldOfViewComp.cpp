...
void UFieldOfViewComp::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	for (int i = 0; i < amountOfTraces; i++)
	{
		...
		FHitResult Hit = Trace(startLocation, startLocation + DirFromAngle(angleOfTrace) * (viewDistance + edgeHitDistance));
		FViewCastRes hit;
		hit = Hit;
		...
		if (Hit.bBlockingHit)
		{
			if (bEnableDebug)
			{
				DrawDebugLine(GetWorld(), startLocation, Hit.ImpactPoint - DirFromAngle(angleOfTrace) * edgeHitDistance, debugColor, false, -1, 0, lineThickness);
				DrawDebugBox(GetWorld(), Hit.ImpactPoint - DirFromAngle(angleOfTrace) * edgeHitDistance, FVector(5, 5, 5), debugColor);
			}
			vertices.Add(Hit.ImpactPoint - DirFromAngle(angleOfTrace) * edgeHitDistance);
		}
		else
		{
			if (bEnableDebug)
			{
				DrawDebugLine(GetWorld(), startLocation, Hit.TraceEnd - DirFromAngle(angleOfTrace) * edgeHitDistance, debugColor, false, -1, 0, lineThickness);
			}
			vertices.Add(Hit.TraceEnd - DirFromAngle(angleOfTrace) * edgeHitDistance);
		}
		oldRay = hit;
	}
	CreateMesh();
...
void UFieldOfViewComp::FindEdge(const FViewCastRes& minRay, const FViewCastRes& maxRay, FVector& outMinPoint, FVector& outMaxPoint)
{
	...
	for (int i = 0; i < resolutionOfEdgeDetection; i++)
	{
		...
		FHitResult Hit = Trace(startLocation, startLocation + DirFromAngle(angle) *  (viewDistance + edgeHitDistance));
		newViewCast = Hit;
		...
		{
			minAngle = angle;
			minPoint = newViewCast.point - DirFromAngle(minAngle) * edgeHitDistance;
		}
		else
		{
			maxAngle = angle;
			maxPoint = newViewCast.point - DirFromAngle(minAngle) * edgeHitDistance;
		}
